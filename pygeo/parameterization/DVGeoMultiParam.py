...
class DVGeometryMultiParam:
    ...
    def addComponent(self, comp, DVGeo, triMesh=None, scale=1.0, bbox={},params=None,bboxForce=None,bboxForceTF=True):
        ...
        if params=='VSP':
            ...
            bboxMax = np.zeros((3, ncomps))
            ic=0
            for c in DVGeo.allComps:
                bbox00= DVGeo.bbox[c]
                bboxMin[:,ic]=bbox00[:,0]
                bboxMax[:,ic]=bbox00[:,1]
                ic+=1
    
            xMin[0] = bboxMin[0,:].min()
            xMax[0] = bboxMax[0,:].max()
            xMin[1] = bboxMin[1,:].min()
            xMax[1] = bboxMax[1,:].max()
            xMin[2] = bboxMin[2,:].min()
            xMax[2] = bboxMax[2,:].max()
                    ...
                    if comp in projList:
                        ...
                        if self.comps[comp].triMesh and self.comps[comp].params=='FFD':
                            ...
                            raise Error(
                                ...
                                + "The point could be shared between VSP and FFD."
                            )
    ...
    def totalSensitivityV1(self, dIdpt, ptSetName, comm=None, config=None):
        """
    ...
    def _computeTotalJacobian(self, ptSetName):
        ...
        for comp in self.compNames:
            ...
            if self.comps[comp].params =="FFD":
                ...
                self.comps[comp].DVGeo._computeSurfJacobian()
            ...
            if self.comps[comp].params =="FFD":
                ...
                if self.comps[comp].DVGeo.pointSets[ptSetName].jac is not None:
                    ...
                    # print('VSP',self.comps[comp].DVGeo.pointSets[ptSetName].nPts,compJ)
                    # do it (kinda) vectorized
                    jac[ptSet.compMapFlat[comp], dvOffset : dvOffset + nDVComp] = compJ[:, :]
